...
async def run(
    ...
    llm_config_raw = args["llm"]
    llm_config = LanguageModelConfig(**llm_config_raw)
    splitter = _get_splitter(llm_config, batch_max_tokens)
    
    # Extract extra fields from raw config that might not be in LanguageModelConfig
    # (e.g., model_name, device for custom embedding models)
    extra_kwargs = {}
    if "model_name" in llm_config_raw:
        extra_kwargs["model_name"] = llm_config_raw["model_name"]
    if "device" in llm_config_raw:
        extra_kwargs["device"] = llm_config_raw["device"]
