...
class LanguageModelConfig(BaseModel):
    ...
    model_name: str | None = Field(
        description="The local model path or name (for custom models like nvembed_embedding).",
        default=None,
    )
    device: str | None = Field(
        description="The device to use for local models (e.g., 'cuda' or 'cpu').",
        default=None,
    )
    ...
    def _validate_encoding_model(self) -> None:
        ...
        For custom embedding models like nvembed_embedding, if encoding_model is empty,
        the model will use its own tokenizer (handled by get_tokenizer).
        ...
        KeyError
            If the model name is not recognized.
        """
        # Skip auto-setting encoding_model for custom embedding models that use their own tokenizer
        if self.type == "nvembed_embedding" and self.encoding_model.strip() == "":
            # Let the model use its own tokenizer, don't try to auto-set encoding_model
            return
